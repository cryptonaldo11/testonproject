...
export async function seedBaseData() {
  ...
  for (const department of allDepts) {
    deptMap[department.name] = department.id;
  }
  ...
  const roleValues = [
    ...
    { name: "manager", description: "Operations manager — monitor attendance, leave, productivity, and alerts", permissions: '["attendance:read","leaves:read","reports:read","alerts:read","medical:read","productivity:read"]' },
  ...
  const hashPass = async (password: string) => bcrypt.hash(password, 10);
  ...
  const usersToSeed = [
    ...
    {
      name: "Noor Aisyah",
      email: "manager@teston.com",
      passwordHash: await hashPass("Manager@123"),
